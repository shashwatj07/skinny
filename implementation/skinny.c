...
void display_matrix(unsigned char state[4][4], int ver)
{
    ...
    if (versions[ver][0]==64)
    {
        for(i = 0; i < 8; i++) input[i] = ((state[(2*i)>>2][(2*i)&0x3] & 0xF) << 4) | (state[(2*i+1)>>2][(2*i+1)&0x3] & 0xF);
         for(i = 0; i < 8; i++) printf("%02x", input[i]);
    }
...
void display_cipher_state(unsigned char state[4][4], unsigned char keyCells[3][4][4], int ver)
{
    ...
    printf("S = ");display_matrix(state,ver);
    for(k = 0; k <(int)(versions[ver][1]/versions[ver][0]); k++)
    {
        printf(" - TK%i = ",k+1); display_matrix(keyCells[k],ver);
    }
}
...
void dec(unsigned char* input, const unsigned char* userkey, int ver)
{
    ...
    #ifdef DEBUG
        printf("DEC - initial state:                     ");display_cipher_state(state,keyCells,ver);printf("\n");
    #endif

	for(i = versions[ver][2]-1; i >=0 ; i--){
        MixColumn_inv(state);
            #ifdef DEBUG
            printf("DEC - round %.2i - after MixColumn_inv:    ",i);display_cipher_state(state,keyCells,ver);printf("\n");
            #endif
        ShiftRows_inv(state);
            #ifdef DEBUG
            printf("DEC - round %.2i - after ShiftRows_inv:    ",i);display_cipher_state(state,keyCells,ver);printf("\n");
            #endif
        AddKey_inv(state, keyCells, ver);
            #ifdef DEBUG
            printf("DEC - round %.2i - after AddKey_inv:       ",i);display_cipher_state(state,keyCells,ver);printf("\n");
            #endif
        AddConstants(state, i);
            #ifdef DEBUG
            printf("DEC - round %.2i - after AddConstants_inv: ",i);display_cipher_state(state,keyCells,ver);printf("\n");
            #endif
        if (versions[ver][0]==64)
            SubCell4_inv(state);
        else
            SubCell8_inv(state);
            #ifdef DEBUG
            printf("DEC - round %.2i - after SubCell_inv:      ",i);display_cipher_state(state,keyCells,ver);printf("\n");
            #endif
	}

	#ifdef DEBUG
        printf("DEC - final state:                       ");display_cipher_state(state,keyCells,ver);printf("\n");
    #endif
...
int main() {
    ...
    printf("Enter the SKINNY Variant to test\n");
    for (i=0; i<(sizeof(versions)/sizeof(*versions));i++) {
        printf("%d: SKINNY-%d-%d\n", i+1, versions[i][0], versions[i][1]);
    }
    int ver;
    scanf("%d", &ver);
    printf("Running Skinny-%d-%d\n", versions[ver-1][0], versions[ver-1][1]);
    TestVectors(ver);
